import React, {useEffect, useState} from 'react';
import breadcrumbimg from '../../assets/images/update_ad_bg.jpg';
import AddPrice from "../../components/addlisting/AddPrice";
import GeneralInfo from "../../components/addlisting/GeneralInfo";
import PhotoUploader from "../../components/addlisting/PhotoUploader";
import Breadcrumb from "../../components/common/Breadcrumb";
import GeneralHeader from "../../components/common/GeneralHeader";
import ScrollTopBtn from "../../components/common/ScrollTopBtn";
import Footer from "../../components/common/footer/Footer";
import {useLocation, useNavigate} from 'react-router-dom';
import {postAdvertisementValidation} from "../../utils/validations/validation";
import {findObject, showError} from "../../utils/util";
import {postAdvertisementFormErrors} from "../../utils/validations/error";
import * as advertisementApi from '../../utils/api/advertisement';
import {categories} from "../../const/dropdownData";

const states = {
    breadcrumbimg: breadcrumbimg
}

function UpdateListing(props) {
    const navigate = useNavigate();
    const [error, setError] = useState(postAdvertisementFormErrors)
    const location = useLocation();
    const {state} = location
    const [tempData, setTempData] = useState(null)
    const [advertisementData, setAdvertisementData] = useState({
        advertisementId: null,
        title: null,
        description: null,
        category: null,
        isLimitedTimeOffer: false,
        dateRange: null,
        images: [],
        price: ''
    });

    useEffect(() => {
        setTempData(state)
        setAdvertisementData({
            advertisementId: state?._id,
            title: state?.title,
            description: state?.description,
            category: findObject(categories, state?.category),
            isLimitedTimeOffer: state?.isLimitedTimeOffer,
            dateRange: state?.isLimitedTimeOffer ? [new Date(state?.offerStartDate), new Date(state?.offerEndDate)] : null,
            images: state?.images,
            price: state?.price
        })
    }, []);

    const handleUpdateListing = async () => {
        const res = postAdvertisementValidation(advertisementData)
        setError(res)

        for (const key in res) {
            if (res[key]) {
                showError()
                return
            }
        }

        await onUpdateListing()
    }

    const onUpdateListing = async () => {
        const data = {
            advertisementId: advertisementData.advertisementId,
            title: advertisementData.title,
            description: advertisementData.description,
            category: advertisementData.category.value,
            isLimitedTimeOffer: advertisementData.isLimitedTimeOffer ? 1 : 0,
            offerStartDate: advertisementData.isLimitedTimeOffer ? advertisementData.dateRange[0] : null,
            offerEndDate: advertisementData.isLimitedTimeOffer ? advertisementData.dateRange[1] : null,
            images: JSON.stringify(advertisementData.images) === JSON.stringify(tempData.images) ? [] : advertisementData.images,
            price: advertisementData.price
        }

        const res = await advertisementApi.updateAdvertisement(data)
        if (res) {
            if (tempData?.paymentStatus === 'PENDING') {
                navigate('/listing-confirmation', {
                    state: {
                        vendor: {
                            name: tempData?.vendorName,
                            email: tempData?.vendorEmail,
                            mobileNumber: tempData?.vendorMobileNumber
                        },
                        advertisement: res
                    }
                })
            } else {
                navigate('/dashboard')
            }
        }
    }

    const setAdvertisementFormData = (name, data) => {
        setAdvertisementData({
            ...advertisementData,
            [name]: data
        })
    }

    return (
        <main className="add-listing">
            {/* Header */}
            <GeneralHeader/>

            {/* Breadcrumb */}
            <Breadcrumb CurrentPgTitle="Update Your Advertisement" img={states.breadcrumbimg}/>

            {/* Add Listing */}
            <section className="add-listing-area padding-top-40px padding-bottom-100px">
                <div className="container">
                    <div className="row">
                        <div className="col-lg-9 mx-auto">
                            <GeneralInfo data={advertisementData} onSetDataHandler={setAdvertisementFormData}
                                         error={error}/>

                            <PhotoUploader data={advertisementData} onSetDataHandler={setAdvertisementFormData}
                                           error={error}/>

                            <AddPrice data={advertisementData} onSetDataHandler={setAdvertisementFormData}
                                      error={error}/>

                            <div className="billing-form-item p-0 border-0 mb-0 shadow-none">
                                <div className="billing-content p-0">
                                    <div className="btn-box mt-4">
                                        <button className="theme-btn border-0" onClick={handleUpdateListing}>Update
                                            listing
                                        </button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>

            {/* Newsletter */}
            {/*<NewsLetter newsLetterContent={sectiondata.calltoactions.newsletters} />*/}

            {/* Footer */}
            <Footer/>

            <ScrollTopBtn/>

        </main>
    );
}

export default UpdateListing;
